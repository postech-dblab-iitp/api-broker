...
#define DEFAULT_ADMIN_LOG_FILE		"log/api/api_broker.log"
...
static const char SECTION_NAME[] = "api_broker";
...
#define INI_GETSTR_CHK(str, ini, sec, key, def, lineno) \
  ...
  for (i = 0; i < ini->size; i++)
      ...
      for (j = 0; j < broker_keywords_size; j++)
	{
      ...
      if (!found)
	{
	  PRINTERROR ("api_broker.conf: invalid keyword '%s' (%d)\n", key, ini->lineno[i]);
	  ini_parser_free (ini);
	  return -1;
	}
    }
...
broker_config_dump (FILE * fp, const T_BROKER_INFO * br_info, int num_broker, int br_shm_id)
{
  ...
  fprintf (fp, "#\n# api_broker.conf\n#\n\n");
  fprintf (fp, "# api broker parameters were loaded from the files\n");
