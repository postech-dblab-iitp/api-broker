...
s62_get_property_from_catalog (char *labelname, int type, S62_PROPERTY ** property)
{
  ...
  if (strcmp (labelname, "node_sample") == 0)
    {
      property_res =
	add_property (property_res, labelname, 1, (char *) "node_property1", 1, DB_TYPE_STRING, DB_TYPE_STRING, 20, 0);
      if (property_res != NULL)
	num_results++;
      property_res =
	add_property (property_res, labelname, 1, (char *) "node_property2", 2, DB_TYPE_INTEGER, DB_TYPE_INTEGER, 8, 0);
      if (property_res != NULL)
	num_results++;
    }
    ...
    {
      property_res =
	add_property (property_res, labelname, 2, (char *) "edge_property1", 1, DB_TYPE_STRING, DB_TYPE_STRING, 20, 0);
      if (property_res != NULL)
	num_results++;
      property_res =
	add_property (property_res, labelname, 2, (char *) "edge_property2", 2, DB_TYPE_INTEGER, DB_TYPE_INTEGER, 8, 0);
      if (property_res != NULL)
	num_results++;
    }
  ...
  if (stmt != NULL)
    {
      ...
      stmt->property =
	add_property (stmt->property, (char *) "node_sample", 1, (char *) "node_property", 1, DB_TYPE_STRING,
		      DB_TYPE_STRING, 20, 0);
      if (stmt->property != NULL)
	stmt->num_property++;
      stmt->property =
	add_property (stmt->property, (char *) "edge_sample", 2, (char *) "edge_property", 2, DB_TYPE_STRING,
		      DB_TYPE_STRING, 20, 0);
      if (stmt->property != NULL)
	stmt->num_property++;
      stmt->property =
	add_property (stmt->property, (char *) "", 0, (char *) "node+edge", 3, DB_TYPE_STRING, DB_TYPE_STRING, 40, 0);
      if (stmt->property != NULL)
	stmt->num_property++;
    }
...
s62_get_string (S62_RESULTSET * resultset, int idx)
{
  ...
  if (resultset == NULL || resultset->position <= 0 || resultset->position > resultset->num_row || idx < 0 || idx > 2)
    {
      return NULL;
    }
